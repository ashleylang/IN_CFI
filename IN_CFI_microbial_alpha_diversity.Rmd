...
shal_AM_E1 = ggeffect(shal_am_invsim_soil, terms = c("pH"), type = "random")
shal_Fun_E1= ggeffect(shal_fungi_invsim_soil, terms = c("pH"), type = "random")
shal_Bact_E1= ggeffect(shal_bacteria_invsim_soil, terms = c("pH"), type = "random")
shal_ECM_E1= ggeffect(shal_ecm_invsim_soil, terms=c("pH"), type="random")

deep_AM_E1 = ggeffect(deep_am_invsim_soil, terms = c("pH"), type = "random")
deep_Fun_E1= ggeffect(deep_fungi_invsim_soil, terms = c("pH"), type = "random")
deep_Bact_E1= ggeffect(deep_bacteria_invsim_soil, terms = c("pH"), type = "random")
deep_ECM_E1= ggeffect(deep_ecm_invsim_soil, terms=c("pH"), type="random")
...
b <- ggplot() +
  ...
  geom_line(data = shal_Fun_E1, aes(x = x, y = predicted, color="0-5 cm"), size = 1.5) +
...
#  geom_line(data = shal_ECM_E1, aes(x = x, y = predicted), size = 1.5, color="blue",linetype="dashed") +
  geom_point(data = deep_ECM_diversity_data, aes(x = pH, y = deep_ECM_invsimpson, color="5-10 cm"), size=3) +
#  geom_line(data = deep_ECM_E1, aes(x = x, y = predicted), size = 1.5, color="red",linetype="dashed") +
...
ggsave("Figure_2.tiff", width = 10.5 , height = 8, units = c("in"))
...
```{r stand_age_fig, echo=F, message=F, warning=F}
 shal_Bact_E2 = ggeffect(shal_bacteria_invsim_plantprod, terms = c("stand_age_years"), type = "random")
 shal_Fun_E2= ggeffect(shal_fungi_invsim_plantprod, terms = c("stand_age_years"), type = "random") #not significant
 shal_AM_E2= ggeffect(shal_am_invsim_plantprod, terms = c("stand_age_years"), type = "random")
 shal_EM_E2= ggeffect(shal_ecm_invsim_plantprod, terms=c("stand_age_years"), type="random")
 
 deep_Bact_E2 = ggeffect(deep_bacteria_invsim_plantprod, terms = c("stand_age_years"), type = "random")
 deep_Fun_E2= ggeffect(deep_fungi_invsim_plantprod, terms = c("stand_age_years"), type = "random")
 deep_AM_E2= ggeffect(deep_am_invsim_plantprod, terms = c("stand_age_years"), type = "random")
 deep_EM_E2= ggeffect(deep_ecm_invsim_plantprod,  terms=c("stand_age_years"), type="random")#not significant
...
```{r EM_VAI fig}
shal_ECM_E4= ggeffect(shal_ecm_invsim_plantstructure, terms = c("VAI"), type = "random")
...
labs(x= "Vegetation Area Index", y="Alpha diversity of EM fungi", color="Soil Depth" )+
   scale_color_manual(values=colors)+
  theme_cowplot()+  
   geom_text(aes(x=0.25, y=15.5, label= "p= 0.009"))

      ggsave("EM_VAI_partial.pdf", width = 8 , height = 6, units = c("in")) 
...
cor_df=deep_fungi_environmental_data %>% 
  ...
  drop_na()
M=cor(cor_df)  
colnames(M)= c("MAT", "TAP", "Latitude", "Longitude", "Soil % N", "Soil % C", "Soil C:N", "Soil pH", "Soil % Fe", "Stand age", "Total basal area (2020)", "Basal area increment", "Vert SD", "VAI", "VCI", "Tree richness", "AM dominance", "AM tree richness", "EM tree richness")
rownames(M)=c("MAT", "TAP", "Latitude", "Longitude", "Soil % N", "Soil % C", "Soil C:N", "Soil pH", "Soil % Fe", "Stand age", "Total basal area (2020)", "Basal area increment", "Vert SD", "VAI", "VCI", "Tree richness", "AM dominance", "AM tree richness", "EM tree richness")
