...
tab_model(shallow_am_2, deep_am_2, shallow_em_2, deep_em_2, shallow_bacteria_2, deep_bacteria_2, shallow_fungi_2, deep_fungi_2, show.se = TRUE, show.ci = FALSE, show.std = "std2", digits = 2, digits.re = 2, show.stat = T, show.df = T)
...
tab_model(shallow_am_2, deep_am_2, shallow_em_2, deep_em_2, shallow_bacteria_2, deep_bacteria_2, shallow_fungi_2, deep_fungi_2, show.se = TRUE, show.ci = FALSE, show.std = "std2", digits = 2, digits.re = 2, show.stat=TRUE, show.df=TRUE)
...
#model for bacteria- testing if nonlinear fit is better
#yes for shal bacteria, no for deep
# yes for both shal and deep fungi 
#no for shal and deep AM
# no for shal and deep ECM
# a1 <- lm(deep_AM_invsimpson~ pH, data= deep_AM_diversity_data)
# a2 <- lm(deep_AM_invsimpson ~ poly(pH,2), data= deep_AM_diversity_data)
# summary(a1)
# summary(a2)
...
shal_ECM_EX= ggeffect(shallow_em_2, terms = c("stand_age_years"), type = "random")
...
a <- ggplot()+
  ...
  scale_color_manual(values=colors)+
  theme_cowplot()+
      theme(axis.title=element_text(size=18),axis.text=element_text(size=14),legend.title=element_text(size=14))+
  geom_text(aes(x=0.68, y=16.5, label="0-5 cm: r = 0.19; p = 0.023"), size=5)


#now ECM vs stand age
a2 <- ggplot()+
  geom_point(data = shal_ECM_diversity_data, aes(x = stand_age_years, y = shal_ECM_invsimpson, color="0-5 cm"), size=3) +
  geom_line(data = shal_ECM_EX, aes(x = x, y = predicted, color="0-5 cm"), size = 1.5) +
  geom_point(data = deep_ECM_diversity_data, aes(x = stand_age_years, y = deep_ECM_invsimpson, color="5-10 cm"), size=3) +
  labs(x= "Stand Age (years)", y="Alpha diversity of EM fungi", color="Soil Depth" )+
  scale_color_manual(values=colors)+
  theme_cowplot()+
      theme(axis.title=element_text(size=18), axis.text=element_text(size=14), legend.title=element_text(size=14))+
  geom_text(aes(x=43, y=16.5, label="0-5 cm: r = 0.21; p = 0.017"), size=5)
...
d <- ggplot()+
  ...
  ggarrange(a,a2, labels=c("a", "b"), nrow=1, ncol=2, legend="right", common.legend=TRUE)
      ggsave("ECM_a_div_fig.pdf", width = 12 , height = 5, units = c("in")) 
...
#r=0.24
#  p=0.16
summary(lm(AMtreerichness~deep_AM_invsimpson, data=deep_AM_diversity_data))
...
soil_summary=shal_bacteria_environmental_data %>% 
  summarize()

summary(shal_bacteria_environmental_data)
```
